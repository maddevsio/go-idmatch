	...
	for _, field := range t.Structure {
		...
		for _, item := range blocks {
			...
			if d := math.Sqrt(float64((item.x-field.X)*(item.x-field.X)) + float64((item.y-field.Y)*(item.y-field.Y))); d < min && d < 0.1 {
				...
				gocv.PutText(&img, field.Field+"("+field.Type+")", image.Point{int(float64(img.Cols()) * item.x), int(float64(img.Rows()) * item.y)}, gocv.FontHersheyPlain, 1, color.RGBA{0, 0, 0, 255}, 1)
				// fmt.Println()
			}
		}
	}
